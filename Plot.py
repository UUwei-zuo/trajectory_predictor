...
#         # Change the legend to correctly reflect what's being plotted
#         legend = ["KalmanNet", "Ground Truth", "Kalman Filter"]
#         font_size = 14
#         T_test = KNet_out[0].size()[1]
#         x_plt = range(0, T_test)
#
#         if dim == 0:  # position
#             plt.plot(x_plt, KNet_out[0][0, :].detach().cpu().numpy(), label=legend[0])
#             plt.plot(x_plt, test_target[0][0, :].detach().cpu().numpy(), label=legend[1])
#             plt.plot(x_plt, KF_out[0][0, :].detach().cpu().numpy(), label=legend[2])
...
#             plt.clf()
#         elif dim == 1:  # velocity
#             plt.plot(x_plt, KNet_out[0][1, :].detach().cpu().numpy(), label=legend[0])
#             plt.plot(x_plt, test_target[0][1, :].detach().cpu().numpy(), label=legend[1])
#             plt.plot(x_plt, KF_out[0][1, :].detach().cpu().numpy(), label=legend[2])
#             plt.legend(fontsize=font_size)
#             plt.xlabel('t', fontsize=font_size)
...
#             plt.plot(x_plt, KNet_out[0][2, :].detach().cpu().numpy(), label=legend[0])
#             plt.plot(x_plt, test_target[0][2, :].detach().cpu().numpy(), label=legend[1])
#             plt.plot(x_plt, KF_out[0][2, :].detach().cpu().numpy(), label=legend[2])
#             plt.legend(fontsize=font_size)
#             plt.xlabel('t', fontsize=font_size)
...
# Legend definitions
Klegend = ["Unsupervised KalmanNet - Train", "Unsupervised KalmanNet - Validation", "Unsupervised KalmanNet - Test",
           "Kalman Filter"]
RTSlegend = ["RTSNet - Train", "RTSNet - Validation", "RTSNet - Test", "RTS Smoother", "Kalman Filter"]
ERTSlegend = ["RTSNet - Train", "RTSNet - Validation", "RTSNet - Test", "RTS", "EKF"]
error_evol = ["KNet Empirical Error", "KNet Covariance Trace", "KF Empirical Error", "KF Covariance Trace",
              "KNet Error Deviation", "EKF Error Deviation"]
...
class BasePlotter:
    ...
    def __init__(self, folderName, modelName):
        ...
        Args:
            folderName: Directory to save plots
            modelName: Name of the model being plotted
        """
        self.folderName = folderName
        self.modelName = modelName
    ...
    def plot_histogram(self, data_dict, title_suffix="Histogram [dB]", filename="plt_hist_dB"):
        ...
        """
        fileName = self.folderName + filename
        ...
        ax = sns.displot(
            ...
            palette=["blue", "orange", "g"],  # Default color palette
            linewidth=1,
        )

        plt.title(f"{self.modelName}: {title_suffix}")
        plt.xlabel('MSE Loss Value [dB]')
        plt.ylabel('Percentage')
        sns.move_legend(ax, "upper right")
        plt.grid(True)
    ...
    def plot_training_progress(self, epochs, train_data, val_data, test_avg,
                               ...
                               legends=None, title_suffix="MSE Loss [dB] - per Epoch"):
        """
        ...
        Args:
            ...
            legends: List of legend labels to use
            title_suffix: Suffix for the plot title
        """
        if legends is None:
            legends = Klegend
        ...
        # Train
        plt.plot(x_plt, train_data[:epochs], KColor[0], label=legends[0])

        # CV
        plt.plot(x_plt, val_data[:epochs], KColor[1], label=legends[1])

        # Test
        plt.plot(x_plt, test_avg * torch.ones(epochs), KColor[2], label=legends[2])

        # First baseline (e.g., RTS or KF)
        if baseline_data is not None:
            plt.plot(x_plt, baseline_data * torch.ones(epochs), KColor[3],
        ...
        if second_baseline is not None:
            plt.plot(x_plt, second_baseline * torch.ones(epochs), KColor[4],
        ...
        plt.ylabel('MSE Loss Value [dB]', fontsize=fontSize)
        plt.grid(True)
        plt.title(f"{self.modelName}: {title_suffix}", fontsize=fontSize)
        plt.savefig(fileName)
        plt.close()

    def plot_trajectory(self, test_target, method1_out, method2_out, dim, file_name,
                        method1_label="Method 1", method2_label="Method 2", truth_label="Ground Truth"):
        """
        Plot trajectory comparison for different methods

        Args:
            test_target: Ground truth trajectory data
            method1_out: Output from first method (e.g., KalmanNet)
            method2_out: Output from second method (e.g., Kalman Filter)
            dim: Dimension to plot (0=position, 1=velocity, 2=acceleration)
            file_name: Name for the saved file (with path)
            method1_label: Label for the first method
            method2_label: Label for the second method
            truth_label: Label for the ground truth
        """
        legend = [method1_label, truth_label, method2_label]
        font_size = 14
        T_test = method1_out[0].size()[1]
        x_plt = range(0, T_test)
        ...
        try:
            data1 = method1_out[0][dim, :].detach().cpu().numpy()
            data_truth = test_target[0][dim, :].detach().cpu().numpy()
            data2 = method2_out[0][dim, :].detach().cpu().numpy()
        except:
            # Fallback if .cpu() fails (data already on CPU)
            data1 = method1_out[0][dim, :].detach().numpy()
            data_truth = test_target[0][dim, :].detach().numpy()
            data2 = method2_out[0][dim, :].detach().numpy()

        plt.figure(figsize=(10, 6))
        plt.plot(x_plt, data1, label=legend[0])
        plt.plot(x_plt, data_truth, label=legend[1])
        plt.plot(x_plt, data2, label=legend[2])
        plt.legend(fontsize=font_size)
        plt.xlabel('t', fontsize=font_size)
...
class Plot_KF(BasePlotter):
    """Plotting utilities for Kalman Filter results"""

    def NNPlot_epochs(self, N_Epochs_plt, MSE_KF_dB_avg,
                      MSE_test_dB_avg, MSE_cv_dB_epoch, MSE_train_dB_epoch):
        """Plot training progress for KalmanNet"""
    ...
    @staticmethod
    def KFPlot(res_grid):
        """Plot Kalman Filter grid results"""
        plt.figure(figsize=(50, 20))
        x_plt = [-6, 0, 6]
    ...
    def NNPlot_test(MSE_KF_linear_arr, MSE_KF_linear_avg, MSE_KF_dB_avg,
                    MSE_test_linear_arr, MSE_test_linear_avg, MSE_test_dB_avg):
        """Plot test results for KalmanNet"""
        N_Epochs_plt = 100
        ...
        plt.figure(figsize=(50, 20))
        sns.distplot(MSE_test_linear_arr, hist=False, kde=True, kde_kws={'linewidth': 3}, color='g', label='KalmanNet')
        sns.distplot(MSE_KF_linear_arr, hist=False, kde=True, kde_kws={'linewidth': 3}, color='b',
                     label='Kalman Filter')
        plt.title("Histogram [Linear]")
        plt.savefig('plt_hist_linear')
        ...
        plt.figure(figsize=(50, 20))
        sns.distplot(10 * torch.log10(MSE_test_linear_arr), hist=False, kde=True, kde_kws={'linewidth': 3}, color='g',
                     label='KalmanNet')
        sns.distplot(10 * torch.log10(MSE_KF_linear_arr), hist=False, kde=True, kde_kws={'linewidth': 3}, color='b',
                     label='Kalman Filter')
        plt.title("Histogram [dB]")
        plt.savefig('plt_hist_dB')
...
class Plot_RTS(BasePlotter):
    """Plotting utilities for RTS Smoother results"""

    def NNPlot_epochs(self, N_MiniBatchTrain_plt, BatchSize, MSE_KF_dB_avg, MSE_RTS_dB_avg,
                      MSE_test_dB_avg, MSE_cv_dB_epoch, MSE_train_dB_epoch):
        """Plot training progress for RTSNet"""
        N_Epochs_plt = np.floor(N_MiniBatchTrain_plt / BatchSize).astype(int)  # number of epochs

        # Process epoch data to match epochs
        train_data = MSE_train_dB_epoch[np.linspace(0, BatchSize * (N_Epochs_plt - 1), N_Epochs_plt)]
        val_data = MSE_cv_dB_epoch[np.linspace(0, BatchSize * (N_Epochs_plt - 1), N_Epochs_plt)]
        ...
        self.plot_training_progress(
            ...
            val_data=val_data,
            test_avg=MSE_test_dB_avg,
            baseline_data=MSE_RTS_dB_avg,
            baseline_label=RTSlegend[3],
            second_baseline=MSE_KF_dB_avg,
            second_label=RTSlegend[4],
            legends=RTSlegend
        )

    def NNPlot_Hist(self, MSE_KF_linear_arr, MSE_RTS_data_linear_arr, MSE_RTSNet_linear_arr):
        """Plot histogram of MSE values for RTSNet, KF and RTS"""
        self.plot_histogram({
            self.modelName: MSE_RTSNet_linear_arr,
            'Kalman Filter': MSE_KF_linear_arr,
            'RTS Smoother': MSE_RTS_data_linear_arr
        })

    def KF_RTS_Plot_Linear(self, r, MSE_KF_RTS_dB, PlotResultName):
        """Plot KF vs RTS vs RTSNet performance across different noise levels"""
        fileName = self.folderName + PlotResultName
        plt.figure(figsize=(25, 10))
        x_plt = 10 * torch.log10(1 / r ** 2)

        plt.plot(x_plt, MSE_KF_RTS_dB[0, :], '-^', color='orange', linewidth=1, markersize=12, label=r'2x2, KF')
        plt.plot(x_plt, MSE_KF_RTS_dB[1, :], '--go', markerfacecolor='none', linewidth=3, markersize=12,
                 label=r'2x2, RTS')
        plt.plot(x_plt, MSE_KF_RTS_dB[2, :], '-bo', linewidth=1, markersize=12, label=r'2x2, RTSNet')
    ...
    def rotate_RTS_Plot_H(self, r, MSE_RTS_dB, rotateName):
        """Plot RTS performance with rotated H matrix"""
        fileName = self.folderName + rotateName
        magnifying_glass, main_H = plt.subplots(figsize=[25, 10])
        x_plt = 10 * torch.log10(1 / r ** 2)
        NoiseFloor = -x_plt
        main_H.plot(x_plt, NoiseFloor, '--r', linewidth=2, markersize=12, label=r'Noise Floor')
        main_H.plot(x_plt, MSE_RTS_dB[0, :], '-g^', linewidth=2, markersize=12,
                    label=r'$\mathrm{\frac{q^2}{r^2}}=0$ [dB] , 2x2, RTS Smoother ($\mathbf{H}_{\alpha=0^\circ}$)')
        main_H.plot(x_plt, MSE_RTS_dB[1, :], '-yx', linewidth=2, markersize=12,
                    label=r'$\mathrm{\frac{q^2}{r^2}}=0$ [dB], 2x2, RTS Smoother ($\mathbf{H}_{\alpha=10^\circ}$)')
        main_H.plot(x_plt, MSE_RTS_dB[2, :], '-bo', linewidth=2, markersize=12,
        ...
        ax2.set_xlim(x1, x2)
        ax2.set_ylim(y1, y2)
        ax2.plot(x_plt, NoiseFloor, '--r', linewidth=2, markersize=12)
        ax2.plot(x_plt, MSE_RTS_dB[0, :], '-g^', linewidth=2, markersize=12)
        ax2.plot(x_plt, MSE_RTS_dB[1, :], '-yx', linewidth=2, markersize=12)
        ax2.plot(x_plt, MSE_RTS_dB[2, :], '-bo', linewidth=2, markersize=12)
        ax2.grid(True)
        plt.savefig(fileName)
        plt.close()

    def rotate_RTS_Plot_FHCompare(self, r, MSE_RTS_dB_F, MSE_RTS_dB_H, rotateName):
        """Compare RTS performance with rotated F and H matrices"""
        fileName = self.folderName + rotateName
        plt.figure(figsize=(25, 10))
        x_plt = 10 * torch.log10(1 / r)

        plt.plot(x_plt, MSE_RTS_dB_F[0, :], '-r^',
                 label=r'$\mathrm{\frac{q^2}{r^2}}=0$ [dB], 2x2, RTS Smoother ($\mathbf{F}_{\alpha=0^\circ}$)')
        plt.plot(x_plt, MSE_RTS_dB_F[1, :], '-gx',
                 label=r'$\mathrm{\frac{q^2}{r^2}}=0$ [dB], 2x2, RTS Smoother ($\mathbf{F}_{\alpha=10^\circ}$)')
        plt.plot(x_plt, MSE_RTS_dB_F[2, :], '-bo',
                 label=r'$\mathrm{\frac{q^2}{r^2}}=0$ [dB], 2x2, RTSNet ($\mathbf{F}_{\alpha=10^\circ}$)')
        plt.plot(x_plt, MSE_RTS_dB_H[0, :], '--r^',
                 label=r'$\mathrm{\frac{q^2}{r^2}}=0$ [dB], 2x2, RTS Smoother ($\mathbf{H}_{\alpha=0^\circ}$)')
        plt.plot(x_plt, MSE_RTS_dB_H[1, :], '--gx',
                 label=r'$\mathrm{\frac{q^2}{r^2}}=0$ [dB], 2x2, RTS Smoother ($\mathbf{H}_{\alpha=10^\circ}$)')
        plt.plot(x_plt, MSE_RTS_dB_H[2, :], '--bo',
                 label=r'$\mathrm{\frac{q^2}{r^2}}=0$ [dB], 2x2, RTSNet ($\mathbf{H}_{\alpha=10^\circ}$)')
    ...
    def plotTraj_CA(self, test_target, KF_out, KNet_out, dim, file_name):
        """Plot trajectory comparison for constant acceleration model"""
        self.plot_trajectory(
            test_target=test_target,
            method1_out=KNet_out,
            method2_out=KF_out,
            dim=dim,
            file_name=file_name,
            method1_label="KalmanNet",
            method2_label="Kalman Filter",
            truth_label="Ground Truth"
        )


class Plot_extended(BasePlotter):
    """Extended plotting utilities for more complex models"""

    def NNPlot_epochs(self, N_E, N_MiniBatchTrain_plt, BatchSize, MSE_EKF_dB_avg, MSE_ERTS_dB_avg,
                      MSE_test_dB_avg, MSE_cv_dB_epoch, MSE_train_dB_epoch):
        """Plot training progress for extended models"""
        N_Epochs_plt = np.floor(N_MiniBatchTrain_plt * BatchSize / N_E).astype(int)  # number of epochs
        print(N_Epochs_plt)

        # Process epoch data to match epochs
        train_data = MSE_train_dB_epoch[np.linspace(0, N_MiniBatchTrain_plt - 1, N_Epochs_plt)]
        val_data = MSE_cv_dB_epoch[np.linspace(0, N_MiniBatchTrain_plt - 1, N_Epochs_plt)]
        ...
        self.plot_training_progress(
            ...
            val_data=val_data,
            test_avg=MSE_test_dB_avg,
            baseline_data=MSE_ERTS_dB_avg,
            baseline_label=ERTSlegend[3],
            second_baseline=MSE_EKF_dB_avg,
            second_label=ERTSlegend[4],
            legends=ERTSlegend
        )

    def NNPlot_Hist(self, MSE_EKF_linear_arr, MSE_ERTS_data_linear_arr, MSE_RTSNet_linear_arr):
        """Plot histogram of MSE values for extended models"""
        self.plot_histogram({
            self.modelName: MSE_RTSNet_linear_arr,
            'Kalman Filter': MSE_EKF_linear_arr,
            'RTS Smoother': MSE_ERTS_data_linear_arr
        })

    def NNPlot_epochs_KF_RTS(self, N_MiniBatchTrain_plt, BatchSize, MSE_EKF_dB_avg, MSE_ERTS_dB_avg,
                             MSE_KNet_test_dB_avg, MSE_KNet_cv_dB_epoch, MSE_KNet_train_dB_epoch,
                             MSE_RTSNet_test_dB_avg, MSE_RTSNet_cv_dB_epoch, MSE_RTSNet_train_dB_epoch):
        """Plot training progress comparing KF and RTS approaches"""
        N_Epochs_plt = np.floor(N_MiniBatchTrain_plt / BatchSize).astype(int)  # number of epochs
        ...
        # Process epoch data
        knet_cv = MSE_KNet_cv_dB_epoch[np.linspace(0, BatchSize * (N_Epochs_plt - 1), N_Epochs_plt)]
        rtsnet_cv = MSE_RTSNet_cv_dB_epoch[np.linspace(0, BatchSize * (N_Epochs_plt - 1), N_Epochs_plt)]

        # CV KNet and RTSNet
        plt.plot(x_plt, knet_cv, color=RTSColor[0], linestyle='-', marker='o', label=Klegend[1])
        plt.plot(x_plt, rtsnet_cv, color=RTSColor[1], linestyle='-', marker='o', label=ERTSlegend[1])

        # Test KNet and RTSNet
        plt.plot(x_plt, MSE_KNet_test_dB_avg * torch.ones(N_Epochs_plt),
                 color=RTSColor[0], linestyle='--', label=Klegend[2])
        plt.plot(x_plt, MSE_RTSNet_test_dB_avg * torch.ones(N_Epochs_plt),
                 color=RTSColor[1], linestyle='--', label=ERTSlegend[2])

        # RTS and EKF baselines
        plt.plot(x_plt, MSE_ERTS_dB_avg * torch.ones(N_Epochs_plt), RTSColor[2], label=ERTSlegend[3])
        plt.plot(x_plt, MSE_EKF_dB_avg * torch.ones(N_Epochs_plt), RTSColor[3], label=ERTSlegend[4])

        plt.legend(fontsize=fontSize)
        plt.xlabel('Number of Training Epochs', fontsize=fontSize)
        plt.ylabel('MSE Loss Value [dB]', fontsize=fontSize)
        plt.title(f"{self.modelName}: MSE Loss [dB] - per Epoch", fontsize=fontSize)
        plt.grid(True)
        plt.savefig(fileName)
        plt.close()
    ...
    def plotTrajectories(self, inputs, dim, titles, file_name):
        ...
        # Set up grid layout based on dimension
        if dim == 3:
            gs1 = gridspec.GridSpec(3, 2)
            gs1.update(wspace=0, hspace=0)
        elif dim == 4:
        ...
        for title in titles:
            ...
            if dim == 3:
                ...
                ax.set_axis_off()
                ax.set_title(title, y=y_al,
                             fontdict={'fontsize': 15, 'fontweight': 20, 'verticalalignment': 'baseline'})
                ax.plot(inputs_numpy[0, :], inputs_numpy[1, :], inputs_numpy[2, :], c, linewidth=0.5)
                ...
                ax.set_title(title, pad=10,
                             fontdict={'fontsize': 20, 'fontweight': 20, 'verticalalignment': 'baseline'})

            elif dim == 4:
                # Special case for pendulum plots
                if title == "True Trajectory":
                    target_theta_sample = inputs_numpy[0, 0, :]
